...
const Article = ({ article }) => {
  ...
  return (
    <section className='container max-w-full w-full mb-10'>
      <Head>
        <title>FD | {singleArticle.title}</title>
      </Head>
      ...
      <div className='flex w-screen justify-between relative h-3/4 md:h-75'>
        <div className='bg-blue-600 z-1 items-center justify-center flex w-full md:w-8/12 p10'>
          <h1 className='text-white my-20 md:w-1/2 text-4xl md:text-8xl font-bold'>
            {singleArticle.title}
          </h1>
        </div>
        <img
          src='https://via.placeholder.com/720x765'
          alt=''
          className='hidden md:flex aboslute right-0'
        />
      </div>
